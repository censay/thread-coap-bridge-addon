...
### Fixed
- Stopped the service startup script from deleting `/data/devices.db`, so previously known devices survive add-on restarts and can be recovered without forcing another device announce
- Add-on version updated to `0.6.8`
...
### Added
- CoAP `/announce` listener on the bridge so devices can register first contact proactively
- Device-side attach announce path that multicasts a small identity payload after Thread attach
